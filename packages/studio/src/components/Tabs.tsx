    ...
    <div className={props.className}>
      <div {...tabListProps} ref={ref} className={props.tabListClassName}>
        {[...state.collection].map((item) => (
          <Tab
            activeClassName={props.tabActiveClassName}
      ...
      <TabPanel
        className={props.tabPanelClassName}
...
function Tab({
  ...
  return (
    <div
      {...tabProps}
      ref={ref}
      className={isSelected === true ? activeClassName : className}
    >
      {rendered}
    </div>
  );
...
function TabPanel({
  ...
  className,
  ...props
}: AriaTabPanelProps & { className?: string; state: TabListState<unknown> }) {
  const ref = useRef(null);

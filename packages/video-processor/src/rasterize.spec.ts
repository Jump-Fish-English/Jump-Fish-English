...
import { describe, beforeAll, it, afterEach, beforeEach, expect as vitestExpect, afterAll } from 'vitest';
...
beforeEach(async () => {
  ...
  await page.waitForFunction(() => {
    return 'rasterizeDocument' in window && window.customElements.get('x-animation-player') !== undefined;
  });
});
  ...
  describe('single CSS clip', () => {
    it('should rasterize with correct duration', async () => {
      await pageEval(async () => {
        const result = await (window as unknown as { rasterizeDocument: typeof rasterizeDocument }).rasterizeDocument({
          sources: {
            css: {
        ...
        const duration = await outputVideo.evaluate((el: HTMLVideoElement) => el.duration);
